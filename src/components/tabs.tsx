                        ...
                        <ul className="flex mb-0 list-none flex-wrap flex-row gap-0 border-b-[6px] border-[#7EB72E]" role="tablist">
                            {Tabs_Data.map((item: any, idx: number) => {
                                return <li key={idx} className="text-center">
                                    <span className={"md:w-[12rem] w-[9rem] block py-[14px] md:px-[30px] px-[9px] rounded-tl-md rounded-tr-md md:text-2xl text-sm font-normal orkneyRegular cursor-pointer  " +
                                        (openTab === item.id
                                            ? "text-white bg-Cstm_gren"
                                            : "text-title_clr bg-white hover:bg-Cstm_gren hover:text-white")
                                    }
                                        ...
                                        return <div key={idx} className={openTab === item.id ? "block" : "hidden"} id="link1">
                                            <ul className="grid gap-3">
                                                {item.content?.map((item: any, _idx: number) => {
                                                    return <li key={_idx} className="md:text-4xl text-2xl font-normal orkneyRegular text-title_clr flex items-center gap-3">
